...
#ifndef MULE_ASSET_STORAGE_HPP
#define MULE_ASSET_STORAGE_HPP

#include <mule/asset/Handler.hpp>
#include <mule/concurrent/UnorderedMap.hpp>
...
public:
     ...
     */
    std::shared_future<Handler> GetAsync(KeyType const& key, uint32_t priority = 100);

private:
    friend class templates::Singleton<Storage>;
    ...
    public:
         ...
         */
        void UpdatePriority(KeyType const& key, uint32_t priority);
    };
     ...
     *  @sa CreateHandler()
     */
    std::shared_future<Handler> ProcessAsyncHandlerCreation(KeyType const& key, uint32_t priority);
